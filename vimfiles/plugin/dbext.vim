...
if has("gui_running") && has("menu") && g:dbext_default_menu_mode != 0
    ...
    exec 'vnoremenu <script> '.menuRoot.'.Execute\ SQL\ (Visual\ selection)<TAB>\\se :DBExecVisualSQL<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Execute\ SQL\ (Under\ cursor)<TAB>\\se  :DBExecSQLUnderCursor<CR>'
    exec 'vnoremenu <script> '.menuRoot.'.Execute\ SQL\ TopX\ (Visual\ selection)<TAB>\\sE  :DBExecVisualSQLTopX<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Execute\ SQL\ TopX\ (Under\ cursor)<TAB>\\sE  :DBExecSQLUnderCursorTopX<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Select\ Table<TAB>\\st  :DBSelectFromTable<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Select\ Table<TAB>\\st  <C-O>:DBSelectFromTable<CR>'
    exec 'vnoremenu <script> '.menuRoot.'.Select\ Table<TAB>\\st  :<C-U>exec ''DBSelectFromTable "''.DB_getVisualBlock().''"''<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Select\ Table\ TopX<TAB>\\sT  :DBSelectFromTableTopX<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Select\ Table\ TopX<TAB>\\sT  <C-O>:DBSelectFromTableTopX<CR>'
    exec 'vnoremenu <script> '.menuRoot.'.Select\ Table\ TopX<TAB>\\sT  :<C-U>exec ''DBSelectFromTableTopX "''.DB_getVisualBlock().''"''<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Select\ Table\ Where<TAB>\\stw  :DBSelectFromTableWithWhere<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Select\ Table\ Where<TAB>\\stw  <C-O>:DBSelectFromTableWithWhere<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Select\ Table\ Ask<TAB>\\sta  :DBSelectFromTableAskName<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Select\ Table\ Ask<TAB>\\sta  <C-O>:DBSelectFromTableAskName<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Describe\ Table<TAB>\\sdt  :DBDescribeTable<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Describe\ Table<TAB>\\sdt  <C-O>:DBDescribeTable<CR>'
    exec 'vnoremenu <script> '.menuRoot.'.Describe\ Table<TAB>\\sdt  :<C-U>exec ''DBDescribeTable "''.DB_getVisualBlock().''"''<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Describe\ Table\ Ask<TAB>\\stda  :DBDescribeTableAskName<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Describe\ Table\ Ask<TAB>\\stda  <C-O>:DBDescribeTableAskName<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Describe\ Procedure<TAB>\\sdp  :DBDescribeProcedure<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Describe\ Procedure<TAB>\\sdp  <C-O>:DBDescribeProcedure<CR>'
    exec 'vnoremenu <script> '.menuRoot.'.Describe\ Procedure<TAB>\\sdp  :<C-U>exec ''DBDescribeProcedure "''.DB_getVisualBlock().''"''<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Describe\ Procedure\ Ask<TAB>\\sdpa  :DBDescribeProcedureAskName<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Describe\ Procedure\ Ask<TAB>\\sdpa  <C-O>:DBDescribeProcedureAskName<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Prompt\ Connect\ Info<TAB>\\sbp  :DBPromptForBufferParameters<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Column\ List<TAB>\\slc  :DBListColumn<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Column\ List<TAB>\\slc  <C-O>:DBListColumn<CR>'
    exec 'vnoremenu <script> '.menuRoot.'.Column\ List<TAB>\\slc  :<C-U>exec ''DBListColumn "''.DB_getVisualBlock().''"''<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Table\ List<TAB>\\slt  :DBListTable<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Table\ List<TAB>\\slt  <C-O>:DBListTable<CR>'
    exec 'noremenu  <script> '.menuRoot.'.Procedure\ List<TAB>\\slp  :DBListProcedure<CR>'
    exec 'inoremenu <script> '.menuRoot.'.Procedure\ List<TAB>\\slp  <C-O>:DBListProcedure<CR>'
    exec 'noremenu  <script> '.menuRoot.'.View\ List<TAB>\\slv  :DBListView<CR>'
    exec 'inoremenu <script> '.menuRoot.'.View\ List<TAB>\\slv  <C-O>:DBListView<CR>'
